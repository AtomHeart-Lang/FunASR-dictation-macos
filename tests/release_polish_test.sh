...
assert 'APP_VERSION="1.0.5"' in create_uninstaller
assert 'APP_VERSION="1.0.5"' in (repo / 'build_dmg.sh').read_text(encoding='utf-8')
assert 'APP_VERSION="1.0.5"' in (repo / 'create_launcher.sh').read_text(encoding='utf-8')
