...
- `batch_size_s`: 推理 batch 秒数（默认 `8`）
- `merge_vad`: 是否合并 VAD 分段（默认 `false`）
- `hotwords`: 可选专有词列表（英文逗号分隔，例如 `OpenAI, GitHub, batch_size_s`）
...
- `false`（本项目默认）：保持分段，断句和边界更稳定。
- `true`：合并分段，在部分场景可减少解码开销。

实用建议：
- 按当前项目默认（速度快且实测稳定）：建议保持 `merge_vad = false`。
